...
APP_ID = os.environ.get("69a8f857a6a0fa216be33357")
...
# ✅ AJOUT : stockage de la progression en mémoire
progress_store = {}
...
def process_separation(file_url, mode, separation_id, token):
    ...
    try:
        ...
        progress_store[separation_id] = {"progress": 10, "step": "Téléchargement du fichier"}
        ...
        progress_store[separation_id] = {"progress": 50, "step": "Séparation des instruments"}
        separator.separate_to_file(tmp.name, out_dir)

        # 3. Trouver les fichiers générés
        progress_store[separation_id] = {"progress": 75, "step": "Génération des pistes"}
...
# ✅ AJOUT : endpoint de progression
@app.route("/progress/<separation_id>", methods=["GET"])
def get_progress(separation_id):
    data = progress_store.get(separation_id, {"progress": 0, "step": "Téléchargement du fichier"})
    return jsonify(data)
